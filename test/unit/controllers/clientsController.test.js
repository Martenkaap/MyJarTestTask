...
describe('clientsController', () => {
	...
	it ('#Changeone should change one client))', async () => {
		const req = {
			params: {
		...
		const obj = { 'clientId': clientId.valueOf('clientId'),
			'firstname': req.body.firstname, 'surname': req.body.surname };

		const validator = td.replace('../../../src/helpers/validator');
		td.when(validator.validate(td.matchers.isA(String), obj))
			.thenReturn({ valid: true });

		const clientModel = td.replace('../../../src/models/ClientModel');
		td.when(clientModel.changebyId(req)).thenResolve();

		const ClientsController = require('../../../src/controllers/ClientsController');
		const ChangeOneResult = await ClientsController.changeOne(req);

		expect(ChangeOneResult)
			.to.be.an('object')
